...
# 職人・色の見本(架空)。職人マスターと、外壁色・屋根色・屋根材
crng = random.Random(31)
painters = ['サンプル塗装', 'デモ塗装', 'テスト塗装']
for c in custs:
    c['職人①'] = crng.choice(painters); c['職人①発注'] = round(c['契約金額(万円)'] * 10000 / 1.1 * crng.uniform(0.18, 0.28), -3)
    if crng.random() < 0.3: c['職人②'] = 'ダミー防水'; c['職人②発注'] = crng.choice([80000, 120000, 200000])
    c['外壁①'] = crng.choice(['サンプルグレー', 'デモホワイト', 'テストブラウン', 'ダミーベージュ', 'KP-000', 'N-00(艶消)', 'N-00'])
    if crng.random() < 0.5: c['外壁②'] = crng.choice(['サンプルグレー', 'テストブラウン', 'ダミーブルー'])
    c['屋根材'] = crng.choice(['ダイナミックルーフ遮熱', 'スーパーシャネツサーモSi', 'スーパーガルテクト', 'フックシングル'])
    c['屋根色'] = crng.choice(['デモブラック', 'サンプルグリーン', 'テストグレー', 'ダミーレッド'])
master = [{'職人名(会社名)': n, '種別': '塗装', '正しい名前(表記ゆれの場合)': None, '使用件数': 0} for n in painters] + [{'職人名(会社名)': 'ダミー防水', '種別': 'その他(塗装以外)', '正しい名前(表記ゆれの場合)': None, '使用件数': 0}]
# 契約に至らなかった反響
...
# 反響の地域(見本)。案件ページの地域検索を試せるように
arng = random.Random(41)
for l in leads:
    l['地域'] = arng.choice(['江戸川', '葛飾', '江東', '足立', '墨田', '台東'])
