...
test('案件: 反響のうち「連絡つかず」「成約」以外(まだ動いている・結果待ちのもの)。区分で絞り込める', () => {
  const leads = [
    { 邸名: '甲野様', 反響日: '2026-01-01', 担当: 'A', 区分: '自社', 媒体: 'チラシ', 結果: '成約' }, // 除外
    { 邸名: '乙野様', 反響日: '2026-01-02', 担当: 'A', 区分: '自社', 媒体: 'チラシ', 結果: '不成約' }, // 含む
    { 邸名: '丙野様', 反響日: '2026-01-03', 担当: 'A', 区分: '自社', 媒体: 'チラシ', 結果: '見積り待ち' }, // 含む
    { 邸名: '丁野様', 反響日: '2026-01-04', 担当: 'B', 区分: 'ポータル', 媒体: 'ヌリカエ', 結果: '連絡つかず' }, // 除外
    { 邸名: '戊野様', 反響日: '2026-01-05', 担当: 'B', 区分: 'ポータル', 媒体: 'ヌリカエ', 結果: '' }, // 含む(結果待ち)
    { 邸名: '己野様', 見積日: '2026-01-06', 担当: 'C', 区分: '訪販', 媒体: '訪問', 結果: '現調待ち' }, // 含む
  ];
  const all = E.dealsList(leads);
  assert.equal(all.length, 4);
  assert.deepEqual(all.map((r) => r.name), ['己野様', '戊野様', '丙野様', '乙野様']); // 日付の新しい順
  assert.equal(E.dealsList(leads, { section: '自社' }).length, 2);
  assert.equal(E.dealsList(leads, { section: 'ポータル' }).length, 1);
  assert.equal(E.dealsList(leads, { section: '訪販' }).length, 1);
  assert.deepEqual(E.dealsCounts(leads), { 自社: 2, ポータル: 1, 訪販: 1 });
});
