...
test('分類: 追加・下請け・媒体グループ', () => {
  ...
  assert.equal(E.isAdd({ 集客経路: '訪問', 顧客名: '橘田　太郎邸追' }), true);
  assert.equal(E.isSub({ 集客経路: 'MIRAI' }), true);
  assert.equal(E.isCountable({ 集客経路: 'ヌリカエ', 顧客名: '楠木　花子' }), true);
...
test('案件: 反響のうち「連絡つかず」「成約」「不成約」以外(まだ残っている案件)。区分・地域で絞り込め、今日に近い日付順', () => {
  const leads = [
    { 邸名: '甲野様', 反響日: '2026-01-01', 担当: 'A', 区分: '自社', 媒体: 'チラシ', 結果: '成約', 地域: '江戸川' }, // 除外
    { 邸名: '乙野様', 反響日: '2026-01-02', 担当: 'A', 区分: '自社', 媒体: 'チラシ', 結果: '不成約', 地域: '江戸川' }, // 除外
    { 邸名: '丙野様', 反響日: '2026-01-03', 担当: 'A', 区分: '自社', 媒体: 'チラシ', 結果: '見積り待ち', 地域: '江戸川' }, // 含む
    { 邸名: '丁野様', 反響日: '2026-01-04', 担当: 'B', 区分: 'ポータル', 媒体: 'ヌリカエ', 結果: '連絡つかず', 地域: '葛飾' }, // 除外
    { 邸名: '戊野様', 反響日: '2026-01-05', 担当: 'B', 区分: 'ポータル', 媒体: 'ヌリカエ', 結果: '', 地域: '葛飾' }, // 含む(結果待ち)
    { 邸名: '己野様', 見積日: '2026-01-08', 担当: 'C', 区分: '訪販', 媒体: '訪問', 結果: '現調待ち', 地域: '江東' }, // 含む
  ];
  const all = E.dealsList(leads, { today: '2026-01-06' });
  assert.equal(all.length, 3);
  assert.deepEqual(all.map((r) => r.name), ['戊野様', '己野様', '丙野様']); // 2026-01-06に近い順(1日差, 2日差, 3日差)
  assert.equal(E.dealsList(leads, { section: '自社' }).length, 1);
  assert.equal(E.dealsList(leads, { section: 'ポータル' }).length, 1);
  assert.equal(E.dealsList(leads, { section: '訪販' }).length, 1);
  assert.equal(E.dealsList(leads, { region: '江戸川' }).length, 1);
  assert.equal(E.dealsList(leads, { region: '葛飾' }).length, 1);
  assert.deepEqual(E.dealsCounts(leads), { 自社: 1, ポータル: 1, 訪販: 1 });
  assert.equal(E.dealsList(leads, { person: 'A' }).length, 1); // 丙野様だけ(甲野=成約, 乙野=不成約は除外)
  assert.equal(E.dealsList(leads, { person: 'B' }).length, 1);
  assert.deepEqual(E.dealsPersonCounts(leads), { A: 1, B: 1, C: 1 }); // 成約・不成約・連絡つかずは数えない
});
...
test('入出金: 入金日が空の案件だけが対象。同じ苗字は名前つき。入金・出金の残りと地域', () => {
  const data = {
    顧客: [
      { 契約日: '2026-05-01', 顧客名: '橘田　太郎', '契約金額(万円)': 145, 住所: '江戸川区江戸川9-9-9', 足場発注: 200000, '職人①発注': 450000, 入金日: null },
      { 契約日: '2026-06-01', 顧客名: '梅沢　一郎', '契約金額(万円)': 100, 住所: '足立区新田3-1', 入金日: null },
      { 契約日: '2026-06-10', 顧客名: '梅沢　二郎', '契約金額(万円)': 80, 住所: '埼玉県川口市栄町1-1', 入金日: null },
      { 契約日: '2025-01-10', 顧客名: '済藤　三郎', '契約金額(万円)': 90, 住所: '江戸川区一之江1-1', 入金日: '2025-02-01' },
    ],
    入出金: [
      { 日付: null, 種別: '入金', 顧客名: '橘田邸', '金額(円)': 870000 },
      { 日付: '2026-05-20', 種別: '出金', 顧客名: '橘田邸', '金額(円)': 500000 },
      { 日付: '2024-01-01', 種別: '入金', 顧客名: '橘田邸', '金額(円)': 999999 }, // 契約日より前 = 別の案件の入金(数えない)
      { 日付: '2026-06-05', 種別: '入金', 顧客名: '梅沢邸(一郎)', '金額(円)': 300000 },
      { 日付: '2025-02-01', 種別: '入金', 顧客名: '済藤邸', '金額(円)': 900000 },
      { 日付: '2026-07-01', 種別: '入金', 顧客名: '見本違い邸', '金額(円)': 1000 }, // 打ち間違い
    ],
  };
  const cb = E.cashBook(data);
  assert.deepEqual(cb.cases.map((c) => c.name), ['橘田邸', '梅沢邸(一郎)', '梅沢邸(二郎)']);
  const m = cb.cases[0];
...
test('施工地図: 追加工事は同じ邸にまとめる。未完工の邸だけ色分け。丁目ごとに並べる', () => {
  ...
  const custs = E.enrichAll({ 顧客: [
    mk('橘田　太郎', '江戸川区東小岩3-1-1', { 完工日: '2026-03-01' }),
    mk('橘田　太郎邸追', '江戸川区東小岩3-1-1', { 集客経路: '追加' }), // 追加は完工日なし → 橘田邸は未完工
    mk('桐野　花子', '江戸川区東小岩3-9-9', { 完工日: '2026-03-01' }),
    mk('楠木　次郎', '江戸川区東小岩5-2-2'),
    mk('丙野　三郎', '江戸川区南小岩1-1-1', { 完工日: '2026-04-01' }),
    mk('下請　施主', '江戸川区南小岩2-2-2', { 集客経路: 'MIRAI' }),
    mk('丁野　一郎', '架空市1-1-1', { 完工日: '2026-04-01' }),
    mk('柊木　四郎', '江戸川区東小岩3-1-1', { 集客経路: '追加', 顧客名: '別の名前邸追' }), // 住所が同じなら同じ邸にまとめる
  ] });
  const houses = E.mapHouses(custs);
  assert.equal(houses.length, 5); // 橘田(追加込み)・桐野・楠木・丙野・丁野(下請けは出さない)
  const y = houses.find((x) => x.name === '橘田');
  assert.equal(y.members.length, 3); assert.equal(y.done, false); assert.equal(y.hasAdd, true);
  assert.deepEqual(E.mapCities(houses).map((x) => x.city), ['江戸川区', '架空市']);
...
test('発注チェック: 未入金の案件だけを対象に、タスクの完了・未完了をまとめる', () => {
  const data = {
    顧客: [
      { 契約日: '2026-06-01', 顧客名: '橘田邸', 入金日: null },
      { 契約日: '2026-05-01', 顧客名: '梅沢邸', 入金日: '2026-08-01' }, // 入金済みは対象外
      { 契約日: '2026-07-01', 顧客名: '丙田邸', 入金日: null }, // タスクがまだ1件も無い(未作成)案件
      { 契約日: null, 顧客名: '空欄邸', 入金日: null }, // 契約日なしは対象外
    ],
    タスク: [
      { 顧客名: '橘田邸', 契約日: '2026-06-01', 項目: '足場発注', 完了日: '2026-06-10' },
      { 顧客名: '橘田邸', 契約日: '2026-06-01', 項目: '塗料発注', 完了日: null },
      { 顧客名: '梅沢邸', 契約日: '2026-05-01', 項目: '足場発注', 完了日: '2026-05-10' }, // 入金済みなので出ない
    ],
  };
  const board = E.taskBoard(data, { today: '2026-09-19' });
  assert.deepEqual(board.map((b) => b.name), ['橘田邸', '丙田邸']); // 契約日の古い順、入金済み・契約日なしは除く
  const y = board[0];
  assert.equal(y.total, 2); assert.equal(y.doneCount, 1);
  assert.equal(y.items[0].item, '足場発注'); assert.equal(y.items[0].done, true);
  assert.equal(y.items[1].done, false);
  assert.equal(board[1].total, 0); // タスクがまだ作られていない案件は空の一覧のまま出す
});

test('発注タスクの遅れアラート: 契約日・着工日からの期限までにチェックが無い案件をステージごとに出す', () => {
  const data = {
    顧客: [
      // A: 契約から18日、着工はまだ先(10/6)。1週間ステージだけ期限が来ていて、一部未チェック
      { 契約日: '2026-09-01', 顧客名: 'A', 着工日: '2026-10-06', 入金日: null },
      // B: 契約から80日、着工から45日たっている。着工まわりのステージが対象、1週間ステージは全部チェック済み
      { 契約日: '2026-07-01', 顧客名: 'B', 着工日: '2026-08-05', 入金日: null },
      // C: 入金済みなので対象外
      { 契約日: '2026-06-01', 顧客名: 'C', 着工日: '2026-07-06', 入金日: '2026-09-01' },
      // D: タスクが1件も無い(未作成)。着工日も未入力
      { 契約日: '2026-09-10', 顧客名: 'D', 入金日: null },
    ],
    タスク: [
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: '粗利予想', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: '顧客名簿', 完了日: null }, // 未チェック(これだけ残す)
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: 'スキャン', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: '4分割', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: '職人発注', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: '足場発注', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: 'DropBox', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: '年賀状', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: '地図', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: 'マップ', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: 'ドライブ', 完了日: '2026-09-02' },
      { 顧客名: 'A', 契約日: '2026-09-01', 項目: '見本板発注', 完了日: '2026-09-02' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '粗利予想', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '顧客名簿', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: 'スキャン', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '4分割', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '職人発注', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '足場発注', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: 'DropBox', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '年賀状', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '地図', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: 'マップ', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: 'ドライブ', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '見本板発注', 完了日: '2026-07-05' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '見本板届け', 完了日: '2026-07-12' },
      { 顧客名: 'B', 契約日: '2026-07-01', 項目: '車', 完了日: '2026-07-20' }, // 「着工日の1週間前」のうち一部だけ完了
    ],
  };
  const stages = E.buildTaskAlerts(data, '2026-09-19');
  const byKey = Object.fromEntries(stages.map((s) => [s.key, s]));
  assert.deepEqual(stages.map((s) => s.key), ['w1', 'w2', 'before', 'start1', 'startmonth']);

  // A: 契約して1週間(期限9/8)が来ていて「顧客名簿」が未チェック → 出る。着工がまだ先の他ステージには出ない
  // D も契約して1週間の期限(9/17)を過ぎているので同じステージに出るが、Aの方が期限超過が長いので先に並ぶ
  assert.deepEqual(byKey.w1.rows.map((r) => r.name), ['A邸', 'D邸']);
  assert.deepEqual(byKey.w1.rows[0].missing, ['顧客名簿']);
  assert.equal(byKey.before.rows.some((r) => r.name === 'A邸'), false);

  // B: 1週間・2週間ステージはすべてチェック済みなので出ない。着工日の1週間前ステージは「車」以外が未チェックで出る
  assert.equal(byKey.w1.rows.some((r) => r.name === 'B邸'), false);
  assert.equal(byKey.w2.rows.some((r) => r.name === 'B邸'), false);
  assert.deepEqual(byKey.before.rows.map((r) => r.name), ['B邸']);
  assert.deepEqual(byKey.before.rows[0].missing, ['足場越境', '打ち合せ', '打ち合わせ書職人送信', '塗料発注', '足場現調', '挨拶', 'フェンス']);
  // 着工して1週間(着手)・着工して1か月(完工・BeforeAfter)も、期限を過ぎていて未チェックなので出る
  assert.deepEqual(byKey.start1.rows.map((r) => r.name), ['B邸']);
  assert.deepEqual(byKey.startmonth.rows.map((r) => r.name), ['B邸']);
  assert.deepEqual(byKey.startmonth.rows[0].missing, ['完工', 'BeforeAfter']);

  // C(入金済み)はどのステージにも出ない
  for (const s of stages) assert.equal(s.rows.some((r) => r.name === 'C邸'), false);

  // D(タスク未作成・着工日なし)は、契約から1週間・2週間ステージには全項目が未チェックとして出るが、着工日が空の着工まわりのステージには出ない
  assert.deepEqual(byKey.w1.rows.find((r) => r.name === 'D邸').missing, E.TASK_ALERT_STAGES[0].items);
  assert.equal(byKey.before.rows.some((r) => r.name === 'D邸'), false);
});
