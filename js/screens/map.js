...
export function render(ctx) {
  ...
  const chip = (x) => {
    ...
    return h('span', { class: `chip ${x.done ? 'done' : 'open'}`, title: tip }, x.label, x.hasAdd ? h('sup', null, '追') : null);
  };
  const block = (b) => h('div', { class: 'mapblock' },
  ...
  const btn = h('button', { class: 'btn primary', type: 'button', disabled: st.busy || !blocks.length, onclick: async () => {
    ...
    try {
      const blob = await mapPdfBlob({ title: `施工地図 ${st.city}${st.onlyOpen ? '(未完工のみ)' : ''}`, sub: `${blocks.reduce((t, b) => t + b.count, 0)}件 ・ 未完工${blocks.reduce((t, b) => t + b.open, 0)}件`, issued: `${Number(today.slice(0, 4))}年${Number(today.slice(5, 7))}月${Number(today.slice(8, 10))}日`, blocks });
  ...
  return h('div', null,
    h('div', { class: 'head' }, h('div', null, h('h1', null, '施工地図'),
      h('div', { class: 'sub' }, '町名ごと・丁目ごとに、施工した邸を並べています。完工日が空欄の邸は青です'))),
    ...
    h('div', { class: 'legend2' }, h('span', { class: 'chip open' }, '未完工'), h('span', { class: 'chip done' }, '完工'), h('span', { class: 'small muted' }, '「追」は追加工事つきの邸(同じ邸にまとめています)。邸にカーソルを合わせる(スマホは長押し)と住所と日付が出ます')),
